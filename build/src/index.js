...
const generateContext = (opts) => {
    ...
    const _sendAug = async (port, address, augData, timeout, onResponse, onTimeout) => {
        ...
        let promise = net_1.default.send(port, address, stringifiedData);
    ...
    const listen = async (handleData) => {
        ...
        const extractUUIDHandleData = (augData, remote) => {
            ...
            if (responseUUIDMapping[UUID])
                handle = responseUUIDMapping[UUID];
            else
                handle = handleData;
        ...
        const server = await net_1.default.listen(PORT, ADDRESS, extractUUIDHandleData);
        return server;
    };
    const stopListening = (server) => {
        return net_1.default.stopListening(server);
    };
